    ...
    try:
            ...
            try:
                ...
                if existing:
                    conn.execute('''
                        ...
                        SET diagnosis=?, medicines=?, instructions=?, next_appointment_date=?, next_appointment_time=?
                        WHERE appointment_id=? AND hospital_id=?
                    ''', (diagnosis, medicines_str, instructions, next_appointment_date, next_appointment_time, appointment_id, session['hospital_id']))
                else:
                    conn.execute('''
                        INSERT INTO prescriptions (appointment_id, diagnosis, medicines, instructions, next_appointment_date, next_appointment_time, hospital_id)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    ''', (appointment_id, diagnosis, medicines_str, instructions, next_appointment_date, next_appointment_time, session['hospital_id']))
        ...
        if prescription:
            prescription_dict = {
                ...
                'next_appointment_date': prescription['next_appointment_date'],
                'next_appointment_time': prescription['next_appointment_time'],
                'medicines_parsed': []
            }
                ...
                for medicine in prescription['medicines'].split('\n'):
                    try:
                        if medicine.strip():
                            ...
                            if len(parts) == 7:
                                prescription_dict['medicines_parsed'].append({
                                    'name': parts[0],
                                    'dosage': parts[1],
                                    'frequency': parts[2],
                                    'morning': parts[3],
                        ...
                        app.logger.error(f"Error parsing medicine in prescription: {str(e)}")
                        continue
    ...
    try:
        prescription_data = conn.execute('''
            SELECT a.id as appointment_id, a.date, a.time_slot, 
                   ...
                   pr.next_appointment_date, pr.next_appointment_time,
                   s.hospital_name
            FROM appointments a
            JOIN patients p ON a.patient_id = p.id
            JOIN doctors d ON a.doctor_id = d.id
        ...
        if prescription_data['medicines']:
            for medicine in prescription_data['medicines'].split('\n'):
                try:
                    ...
                    app.logger.error(f"Error parsing medicine in print prescription: {str(e)}")
                    continue
...
def edit_doctor(doctor_id):
    ...
    try:
        ...
        # Handle image upload
        image_path = doctor['image_path']  # Keep existing image by default
        if 'image' in request.files:
            file = request.files['image']
            if file and file.filename:
                ...
                if doctor['image_path']:
                    try:
                        os.remove(os.path.join(app.config['UPLOAD_FOLDER'], doctor['image_path']))
                    except:
                        pass
        ...
        conn.execute('''
            UPDATE doctors 
            SET name=?, specialization=?, experience=?, consultation_fee=?, contact=?, bio=?, image_path=?
            WHERE id=? AND hospital_id=?
        ''', (name, specialization, experience, consultation_fee, contact, bio, image_path, doctor_id, session['hospital_id']))
        ...
        app.logger.error(f"Edit doctor error: {str(e)}")
    finally:
        conn.close()
