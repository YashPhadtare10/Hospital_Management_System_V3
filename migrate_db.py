...
def migrate_database():
    ...
    try:
        ...
        if 'hospital_id' not in columns:
            ...
            # Check if next_appointment fields exist
            if 'next_appointment_date' not in columns or 'next_appointment_time' not in columns:
                logger.info("Adding next appointment fields to prescriptions table")
                
                # Add next_appointment_date column if it doesn't exist
                if 'next_appointment_date' not in columns:
                    cursor.execute('ALTER TABLE prescriptions ADD COLUMN next_appointment_date TEXT')
                
                # Add next_appointment_time column if it doesn't exist
                if 'next_appointment_time' not in columns:
                    cursor.execute('ALTER TABLE prescriptions ADD COLUMN next_appointment_time TEXT')
                
                conn.commit()
                logger.info("Added next appointment fields successfully")
            else:
                logger.info("Next appointment fields already exist in prescriptions table")
